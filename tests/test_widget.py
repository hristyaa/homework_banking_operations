...
def test_mask_account_card(user_input, expected):
    """Проверки, что функция корректно распознает и применяет нужный тип маскировки в зависимости от типа входных
    данных"""
    assert mask_account_card(user_input) == expected


def test_mask_account_card_invalid_account_card_without():
